...
function projectPoint(u, v, width, height) {
  ...
  var strata = leftStrata(u, v);
  var plateau = centralPlateau(u, v);
  var basin = lowerBasin(u, v);
  var rings = rightRings(u, v);
  var fan = lowerRightFan(u, v);
  var strataWave = 0.45 + 0.55 * Math.sin(v * 18.0 + u * 7.0 + Math.sin(v * 6.0) * 1.6);

  x -= strata * width * 0.024;
  x += rings * width * 0.008 * Math.cos((u - 0.72) * 24.0);
  x += fan * width * 0.018 * (v - 0.72);

  y -= strata * height * 0.028 * strataWave;
  y -= plateau * height * 0.050;
  y += basin * height * 0.038;
  y -= rings * height * 0.018;
  y -= fan * height * 0.020 * (0.55 + (u - 0.62));
...
function leftStrata(u, v) {
  ...
  var bandMask = smoothstep(0.54, 0.22, u) * smoothstep(0.42, 0.64, v) * smoothstep(0.98, 0.54, v);

  var dx = u - (0.09 + (v - 0.72) * 0.42 + Math.sin(v * 10.5) * 0.020);

  var wave = Math.sin(v * 28.0 + u * 10.0 + Math.sin(v * 6.5) * 2.8);

  var spine = 1.0 - smoothstep(0.015, 0.090, Math.abs(dx));

  var band = 1.0 - smoothstep(0.06, 0.19, Math.abs(wave));

  return Math.max(spine * 0.70, band) * bandMask;
...
function centralPlateau(u, v) {
  ...
  var diamond = 1.0 - smoothstep(0.13, 0.29, dx + dy * 0.82);

  var box = smoothstep(0.34, 0.40, u) * smoothstep(0.70, 0.64, u) *

            smoothstep(0.52, 0.58, v) * smoothstep(0.80, 0.74, v);

  var diagonal = 1.0 - smoothstep(0.03, 0.12, Math.abs((u - 0.40) - (v - 0.58) * 0.92));

  return Math.max(diamond, Math.max(box * 0.92, diagonal * box * 0.55));
...
function lowerBasin(u, v) {
  ...
  var arcWave = 1.0 - smoothstep(0.015, 0.060, Math.abs(Math.sin(d * 22.0)));

  var mask = smoothstep(0.58, 0.70, v) * smoothstep(1.55, 0.28, d);

  var arcGate = smoothstep(0.12, 0.38, u) * smoothstep(0.88, 0.66, u);

  return arcWave * mask * arcGate;
...
function rightRings(u, v) {
  ...
  var rings = 1.0 - smoothstep(0.016, 0.052, Math.abs(Math.sin(d * 27.5)));

  var core = 1.0 - smoothstep(0.00, 0.18, d);

  var mask = smoothstep(1.10, 0.16, d);

  return Math.max(rings * mask, core * 0.58);
...
function lowerRightFan(u, v) {
  ...
  var diag = Math.abs((v - 0.77) - (u - 0.68) * 0.72);

  var band = 1.0 - smoothstep(0.012, 0.060, diag);

  var spoke = 1.0 - smoothstep(0.02, 0.10, Math.abs(Math.sin((u * 20.0 - v * 16.0) * 2.2)));

  var mask = smoothstep(0.64, 0.80, u) * smoothstep(0.62, 0.80, v);

  return Math.max(band, spoke * 0.75) * mask;
...
function drawMesh(ctx, canvas, mesh, time) {
  ...
  function drawEdge(a, b, ua, va, ub, vb) {
    var sampleU = (ua + ub) * 0.5;
    var sampleV = (va + vb) * 0.5;
    var feature = Math.max(getFeature(ua, va), getFeature(ub, vb), getFeature(sampleU, sampleV));
    var depth = smoothstep(0.10, 1.0, sampleV);
    var horizonFade = smoothstep(0.16, 0.42, sampleV);
    var shimmer = 0.88 + 0.12 * Math.sin(time * 0.75 + sampleU * 8.0 + sampleV * 5.0);
    ...
    if (feature > 0.55) {
      ...
      color = 'rgba(196, 129, 31, ' + alpha + ')';
    } else {
      color = 'rgba(146, 92, 18, ' + Math.max(0.16, Math.min(alpha, 0.46)) + ')';
    }
  ...
  for (var j = 0; j < rows; j++) {
    ...
    for (var i = 0; i < cols; i++) {
      ...
      var uv00 = mesh.vertices[i00], uv10 = mesh.vertices[i10];
      var uv01 = mesh.vertices[i01], uv11 = mesh.vertices[i11];

      drawEdge(v00, v10, uv00.u, uv00.v, uv10.u, uv10.v);
      drawEdge(v10, v01, uv10.u, uv10.v, uv01.u, uv01.v);
      drawEdge(v01, v00, uv01.u, uv01.v, uv00.u, uv00.v);

      drawEdge(v10, v11, uv10.u, uv10.v, uv11.u, uv11.v);
      drawEdge(v11, v01, uv11.u, uv11.v, uv01.u, uv01.v);
      drawEdge(v01, v10, uv01.u, uv01.v, uv10.u, uv10.v);
...
function drawLeftStrata(ctx, w, h, time) {
  for (var band = 0; band < 6; band++) {
    ...
    var cp1X = w * 0.12;
    var cp1Y = h * (0.64 + band * 0.016 + Math.sin(time * 0.12 + band) * 0.005);
    var cp2X = w * 0.28;
    var cp2Y = h * (0.66 + band * 0.018);
    var endX = w * 0.39;
    var endY = h * (0.58 + band * 0.030);
    ctx.beginPath();
    ctx.moveTo(startX, startY);
    ctx.bezierCurveTo(cp1X, cp1Y, cp2X, cp2Y, endX, endY);
    strokeGold(ctx, 0.48 + band * 0.045, 1.15 + band * 0.12);
    ctx.stroke();
...
function drawRightRings(ctx, w, h, time) {
  ...
  for (var r = 0; r < 7; r++) {
    ...
    strokeGold(ctx, Math.max(0.38, 0.76 - r * 0.055), 1.45 - r * 0.07);
    ctx.stroke();
  }
}
...
function drawLowerBasin(ctx, w, h, time) {
  ...
  for (var r = 0; r < 7; r++) {
    ...
    strokeGold(ctx, Math.max(0.34, 0.70 - r * 0.052), 1.40 - r * 0.07);
    ctx.stroke();
  }
}
...
function drawCentralPlateau(ctx, w, h, time) {
  ...
  for (var idx = 0; idx < shapes.length; idx++) {
    ...
    ctx.closePath();
    strokeGold(ctx, idx === 0 ? 0.76 : 0.56, idx === 0 ? 1.60 : 1.20);
    ctx.stroke();
  }
  var diagonals = [
    [[0.38,0.60],[0.61,0.73]],
...
function drawLowerRightFan(ctx, w, h, time) {
  ...
  for (var i = 0; i < 14; i++) {
    ...
    ctx.lineTo(x2, y2);
    strokeGold(ctx, 0.34 + tt * 0.38, 0.90 + tt * 0.60);
    ctx.stroke();
  }
}
