...
function drawMesh(ctx, canvas, mesh, time) {
  ...
  for (var j = 0; j < rows; j++) {
    for (var i = 0; i < cols; i++) {
      ...
      var alpha = 0.10 + feature * 0.46 + depth * 0.12;
      alpha *= horizonFade;
      alpha *= shimmer;
      alpha = Math.max(0.055, Math.min(alpha, 0.88));
      
      var color;
      if (feature > 0.75) {
        color = 'rgba(224, 160, 48, ' + (alpha * 0.68) + ')';
      } else if (feature > 0.50) {
        color = 'rgba(210, 146, 34, ' + (alpha * 0.76) + ')';
      } else if (feature > 0.25) {
        color = 'rgba(170, 109, 22, ' + (alpha * 0.68) + ')';
      } else {
        color = 'rgba(126, 78, 10, ' + (alpha * 0.48) + ')';
      }
...
function drawLeftStrata(ctx, w, h, time) {
  for (var band = 0; band < 5; band++) {
    var yBase = h * (0.58 + band * 0.065);
    var amp = h * (0.025 + band * 0.004);
    var phase = band * 0.85 + Math.sin(time * 0.18 + band) * 0.08;
    ctx.beginPath();
    for (var i = 0; i <= 160; i++) {
      var tt = i / 160;
      var x = w * (-0.04 + tt * 0.48);
      var y = yBase + Math.sin(tt * Math.PI * 2.15 + phase) * amp + Math.sin(tt * Math.PI * 5.2 + phase) * amp * 0.35;
      if (i === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y);
    }
    strokeGold(ctx, 0.22 + band * 0.035, 0.8 + band * 0.1);
    ctx.stroke();
  }
}
...
function drawRightRings(ctx, w, h, time) {
  ...
  var cy = h * 0.68;
  for (var r = 0; r < 6; r++) {
    var rx = w * (0.035 + r * 0.033);
    var ry = h * (0.018 + r * 0.025);
    ctx.beginPath();
    for (var i = 0; i <= 220; i++) {
      var a = (i / 220) * Math.PI * 2;
      var wobble = 1 + 0.035 * Math.sin(a * 5 + r * 0.8 + time * 0.05);
      var x = cx + Math.cos(a) * rx * wobble;
      var y = cy + Math.sin(a) * ry * wobble;
      if (i === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y);
    }
    strokeGold(ctx, 0.28 - r * 0.018, 0.9);
    ctx.stroke();
  }
}
...
function drawLowerBasin(ctx, w, h, time) {
  ...
  var cy = h * 0.98;
  for (var r = 0; r < 6; r++) {
    var rx = w * (0.12 + r * 0.055);
    var ry = h * (0.055 + r * 0.032);
    ctx.beginPath();
    var start = Math.PI * 1.05;
    var end = Math.PI * 1.95;
    for (var i = 0; i <= 180; i++) {
      var a = start + (i / 180) * (end - start);
      var wobble = 1 + 0.025 * Math.sin(a * 6 + r + time * 0.04);
      var x = cx + Math.cos(a) * rx * wobble;
      var y = cy + Math.sin(a) * ry * wobble;
      if (i === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y);
    }
    strokeGold(ctx, 0.24 - r * 0.018, 0.85);
    ctx.stroke();
  }
}
...
function drawCentralPlateau(ctx, w, h, time) {
  var shapes = [
    [[0.39,0.61],[0.49,0.56],[0.62,0.59],[0.66,0.68],[0.54,0.73],[0.41,0.70]],
    [[0.44,0.63],[0.51,0.60],[0.59,0.62],[0.61,0.68],[0.53,0.70],[0.45,0.68]]
  ];
  for (var idx = 0; idx < shapes.length; idx++) {
    ...
    ctx.closePath();
    strokeGold(ctx, idx === 0 ? 0.34 : 0.22, idx === 0 ? 1.1 : 0.8);
    ctx.stroke();
  }
  var diagonals = [
    [[0.39,0.61],[0.54,0.73]],
...
function drawLowerRightFan(ctx, w, h, time) {
  ...
  var originY = h * 0.78;
  for (var i = 0; i < 9; i++) {
    var angle = -0.15 + i * 0.08;
    var len = w * (0.18 + i * 0.025);
    ctx.beginPath();
    ctx.moveTo(originX, originY + i * h * 0.018);
    var x2 = originX + Math.cos(angle) * len;
    var y2 = originY + Math.sin(angle) * len + h * 0.16;
    ctx.lineTo(x2, y2);
    strokeGold(ctx, 0.16 + i * 0.012, 0.6);
    ctx.stroke();
  }
}
