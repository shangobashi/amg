  ...
  function boot() {
    ...
    var terrainMat = new THREE.ShaderMaterial({
      ...
      vertexShader: [
        ...
        '  float breath = 0.94 + 0.06 * sin(t * 0.40 + uv.y * 2.0);',
        '  float ripple = sin((p.x * 0.016 + p.y * 0.012) - t * 0.35) * 0.38 * smoothstep(0.08, 1.0, uv.y);',
      ...
      fragmentShader: [
        ...
        '  float h = vIntensity;',
        '  float t = uTime;',
        '  /* Gentle Gaussian-like contour peaks — no harsh abs(sin) sawtooth */',
        '  float raw = sin(h * 0.20 + t * 0.06);',
        '  float contour = 0.5 + 0.5 * (raw * raw * raw * raw); /* sin^4 — smooth peaks */',
        '  /* fwidth derivative smoothing — eliminates triangle-edge shimmer */',
        '  float dw = fwidth(h * 0.20);',
        '  float smoothed = smoothstep(0.0, max(dw * 2.8, 0.06), contour - 0.5) * 0.5 + 0.5 * contour;',
        '  /* Clamp intensity to disciplined range: 0.40–0.78 */',
        '  float intensity = 0.40 + smoothed * 0.38;',
        '  /* Horizon depth fade — foreground dim, horizon bright (depth perception) */',
        '  float horizonFade = 1.0 - smoothstep(0.00, 0.52, vUv.y);',
        '  /* Clean horizontal edge fade */',
        '  float edgeFade = smoothstep(0.00, 0.08, vUv.x) * smoothstep(1.00, 0.92, vUv.x);',
        '  /* Three-level line hierarchy: primary 0.72, secondary 0.52, tertiary 0.36 */',
        '  float lineType = smoothstep(0.58, 0.84, contour); /* primary vs secondary vs tertiary */',
        '  float baseAlpha = mix(0.36, 0.72, lineType);',
        '  /* Alpha = baseAlpha * depth * edgeFade */',
        '  float alpha = baseAlpha * horizonFade * edgeFade * 0.60;',
        '  /* Refined gold palette: restrained, not over-bright */',
        '  vec3 goldDim   = vec3(0.40, 0.26, 0.05);',
        '  vec3 goldMid   = vec3(0.52, 0.34, 0.08);',
        '  vec3 goldBright = vec3(0.64, 0.42, 0.10);',
        '  vec3 col = mix(goldDim, goldMid, smoothed);',
        '  col = mix(col, goldBright, lineType * 0.55);',
    ...
    // A subtle point lattice — mineral-map sparkle, restrained so it doesn't compete with mesh
