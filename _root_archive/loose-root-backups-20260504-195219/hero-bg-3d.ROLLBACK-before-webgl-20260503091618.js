...
(function () {
  ...
  function project3(x, y, z, t) {
    ...
    return {
      x: CX + (x + parX) * s,
      y: H * 0.66 + parY - y * s * 0.72 + (z - 38) * 1.62,
      s: s,
  ...
  function drawPoly(body, t) {
    ...
    if (body.ring) {
      ...
      ctx.fillStyle = grd;
      ctx.beginPath(); ctx.arc(mx, my, 7.5, 0, Math.PI * 2); ctx.fill();
    }
  ...
  function drawTerrain(t) {
    ...
    for (var j = 0; j < rows; j++) {
      for (var i = 0; i < cols; i++) {
        ...
        var alpha = 0.060 + centerBoost * 0.15 + frontBoost * 0.20;
  ...
  function line(a, b, alpha) {
    ...
    ctx.moveTo(a.x, a.y);
    ctx.lineTo(b.x, b.y);
    ctx.strokeStyle = 'rgba(' + GOLD + ',' + Math.min(0.46, alpha).toFixed(3) + ')';
    ctx.lineWidth = 0.82;
    ctx.stroke();
  }
